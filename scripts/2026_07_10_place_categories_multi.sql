-- Nice Place — multi-category support (1–4 categories per place)
-- Idempotent. Does not remove places.category (legacy primary category).
...
create index if not exists place_categories_category_key_idx
  on public.place_categories (category_key);

create index if not exists place_categories_place_category_idx
  on public.place_categories (place_id, category_key);

-- Backfill from legacy places.category
...
-- ---------------------------------------------------------------------------
-- place_update_requests.category_keys
-- ---------------------------------------------------------------------------
alter table public.place_update_requests
  add column if not exists category_keys jsonb;

comment on column public.place_update_requests.category_keys is
  'Ordered JSON array of category_key slugs submitted with an edit request (max 4).';
...
create policy place_categories_select_approved
...
create policy place_categories_insert_own
on public.place_categories
for insert
to authenticated
with check (
...
create policy place_categories_delete_own
on public.place_categories
for delete
to authenticated
using (
...
create policy place_categories_admin_select
on public.place_categories
for select
to authenticated
...
create policy place_categories_admin_update
on public.place_categories
for update
to authenticated
using (public.is_current_user_admin())
...
-- ---------------------------------------------------------------------------
-- RPC: sync categories for a place (admin approval / rejected resubmit)
-- ---------------------------------------------------------------------------
create or replace function public.sync_place_categories(
  p_place_id uuid,
...
begin
  ...
  if p_category_keys is not null then
    insert into public.place_categories (place_id, category_key)
    select p_place_id, key
    from unnest(p_category_keys) as key
    where key is not null and trim(key) <> ''
    on conflict (place_id, category_key) do nothing;
  end if;

  update public.places
  set category = coalesce(p_category_keys[1], category)
  where id = p_place_id;
end;
$$;
