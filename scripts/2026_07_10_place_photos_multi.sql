...
--
-- Existing column image_url is kept (equivalent to photo_url in app docs).
-- Adds order_index for stable photo ordering.

-- ---------------------------------------------------------------------------
-- place_photos: order_index
-- ---------------------------------------------------------------------------
alter table public.place_photos
  add column if not exists order_index integer not null default 0;

-- Backfill order_index from created_at within each place (oldest = 0).
with ranked as (
  select
    id,
    row_number() over (
...
-- Ensure exactly one cover per place where photos exist (prefer is_cover, then lowest order).
...
-- ---------------------------------------------------------------------------
-- place_update_requests: ordered photo URLs for edit approval
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- RLS: admins can select all place_photos (including pending) for review
-- (policy may already exist from 2026_07_04_admin_panel_rls.sql)
-- ---------------------------------------------------------------------------
