...
-- Restrict notification history writes to service role only.
REVOKE INSERT, DELETE ON public.notifications FROM anon, authenticated;
GRANT SELECT, UPDATE ON public.notifications TO authenticated;

-- Restrict type values at the database layer.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
...
-- Dedupe support for like notifications.
...
-- Only allow authenticated users to ensure settings for their own profile.
